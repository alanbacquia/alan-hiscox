...
const FALLBACK_MAIN_LINKS = [
  { id: '1', title: 'Business Insurance', url: '/business-insurance' },
  { id: '2', title: 'Personal Insurance', url: '/personal-insurance' },
  { id: '3', title: 'Claims', url: '/claims' },
  { id: '4', title: 'About Us', url: '/about' },
];

const FALLBACK_UTILITY_LINKS = [
  { id: 'u1', title: 'Small business', url: '/small-business' },
  { id: 'u2', title: 'Larger business', url: '/larger-business' },
  { id: 'u3', title: 'Professionals', url: '/professionals' },
];

const ChevronDown = () => (
  <svg aria-hidden="true" className="size-3 shrink-0" viewBox="0 0 10 6" fill="currentColor">
...
const HiscoxLogo = () => (
  <svg className="h-8 w-auto" viewBox="0 0 165.975 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
...
const Header = ({
  menuName = 'main',
  utilityMenuName = 'utility',
  ctaLabel = 'Make a claim',
  ctaUrl = '/make-a-claim',
  loginLabel = 'Log in',
  loginUrl = '/login',
  localeLabel = 'UK',
  ...
  const { data: mainData, error: mainError, isLoading: mainLoading } = useSWR(
    menuName ? ['menu_items', menuName] : null,
    ([type, id]) => client.getResource(type, id),
  );

  const { data: utilityData, error: utilityError, isLoading: utilityLoading } = useSWR(
    utilityMenuName ? ['menu_items', utilityMenuName] : null,
    ([type, id]) => client.getResource(type, id),
  );

  const mainLinks =
    !mainError && !mainLoading && mainData
      ? Array.from(sortMenu(mainData))
      : FALLBACK_MAIN_LINKS;

  const utilityLinks =
    !utilityError && !utilityLoading && utilityData
      ? Array.from(sortMenu(utilityData))
      : FALLBACK_UTILITY_LINKS;

  const [mobileOpen, setMobileOpen] = useState(false);

  return (
    <header className={cn('sticky top-0 z-50 w-full bg-hiscox-heading', className)}>
      {/* Utility bar */}
      <div className="hidden border-b border-white/10 md:block">
        <div className="mx-auto flex h-11 max-w-[1440px] items-center gap-6 px-5 md:px-20">
          {/* BU descriptor links */}
          <nav aria-label="Business type navigation" className="flex items-center gap-0">
            {utilityLinks.map(({ id, title, url }, i) => (
              <span key={id} className="flex items-center">
                {i > 0 && <span className="mx-3 text-white/30 select-none">|</span>}
                <a
                  href={url}
                  className="text-xs font-medium text-white/70 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-hiscox-red"
                >
                  {title}
                </a>
              </span>
          ...
          <div className="flex flex-1 items-center justify-center px-8">
            <div className="flex w-full max-w-sm items-center rounded border border-white/30 bg-transparent">
              <input
                type="text"
                placeholder={searchPlaceholder}
                aria-label={searchPlaceholder}
                className="h-8 flex-1 bg-transparent px-3 text-xs text-white placeholder:text-white/50 outline-none"
              />
              <button
                type="button"
                aria-label="Search"
                className="flex h-8 w-9 items-center justify-center text-white/70 hover:text-white transition-colors"
              >
                <SearchIcon />
              </button>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className="inline-flex items-center gap-1.5 rounded border border-white/30 px-2.5 py-1 text-xs font-medium text-white/80 transition-colors hover:border-white/60 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-hiscox-red"
            >
              <GlobeIcon />
      ...
      {/* Main nav row */}
      <div className="mx-auto flex h-16 max-w-[1440px] items-center gap-6 px-5 md:px-20">
        <a
          href="/"
          ...
          <ul className="flex items-center gap-1">
            {mainLinks.map(({ id, title, url }) => (
              <li key={id}>
                <a
                  href={url}
                  className="inline-flex items-center gap-1 rounded px-3 py-2 text-sm font-medium text-white/90 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-hiscox-red"
                >
                  {title}
                  <ChevronDown />
                </a>
        ...
        <a
          href={ctaUrl}
          className="hidden md:inline-flex h-9 items-center justify-center rounded-lg bg-hiscox-red px-5 text-sm font-semibold text-white hover:bg-hiscox-red-dark transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white shrink-0"
        >
          {ctaLabel}
        </a>
        ...
        <button
          ...
          className="ml-auto flex size-9 items-center justify-center rounded-lg text-white transition-colors hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-hiscox-red md:hidden"
        >
      ...
      {mobileOpen && (
        <div
          id="mobile-nav"
          className="border-t border-white/10 bg-hiscox-heading px-5 pb-6 md:hidden"
        >
          {/* Search */}
          <div className="pt-4 pb-2">
            <div className="flex items-center rounded border border-white/30 bg-transparent">
              <input
                type="text"
                placeholder={searchPlaceholder}
                aria-label={searchPlaceholder}
                className="h-10 flex-1 bg-transparent px-3 text-sm text-white placeholder:text-white/50 outline-none"
              />
              <button
                type="button"
                aria-label="Search"
                className="flex h-10 w-10 items-center justify-center text-white/70 hover:text-white transition-colors"
              >
                <SearchIcon />
              </button>
            </div>
          ...
          <nav aria-label="Main navigation mobile">
            <ul className="flex flex-col gap-1 pt-2">
              {mainLinks.map(({ id, title, url }) => (
                <li key={id}>
                  <a
                    href={url}
                    className="block rounded px-3 py-2.5 text-sm font-medium text-white/90 transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-hiscox-red"
                  >
                    {title}
                  </a>
                </li>
            ...
            <div className="mt-4 flex flex-col gap-2 border-t border-white/10 pt-4">
              <a
                href={loginUrl}
                className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-white/30 text-sm font-medium text-white/80 transition-colors hover:border-white/60 hover:text-white"
              >
                <LoginIcon />
                {loginLabel}
              </a>
              <a
                href={ctaUrl}
                className="flex h-10 w-full items-center justify-center rounded-lg bg-hiscox-red text-sm font-semibold text-white hover:bg-hiscox-red-dark transition-colors"
              >
                {ctaLabel}
              </a>
            </div>
